...
struct Triangle {
	...
	//for constructing a triangle with sprite as input
	Triangle(Vector3 p1, Vector3 p2, Vector3 p3, olc::Sprite* sprite) {
		points[0] = p1;
		points[1] = p2;
		points[2] = p3;
		copy_points();
	...
	bool line_intersect(Edge3& e) {
		...
		Vector3 i = Math::VectorPlaneIntersect(points[0], normal, e.p[0], e.p[1], t);

		//LOG("edge  ", Math::round2v(e.direction().normalized()));
		//LOG("point ", Math::round2v(i.normalized()), "\n");
		...
		if (ta > Math::round2f(area)) { 
			...
			if (!e.point_on_edge(i)) { return false; }
			return true; 
		}
